...
for segments in 4 8 16; do
  parallel -j 1 -u "conda activate double_env_6;python source/approx_run_cifar10.py --task cifar10 --model resnet18 --epochs 20 --progressive_epochs 8 --batch-size 64 --lr 0.001 --seed 41 --pwl_segments {2} --activation {1} --a $a --b $b --c $c --d $d" ::: "${activations[@]}" ::: $segments
done
