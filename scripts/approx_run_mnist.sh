...
for seed in 41; do
  parallel -j 1 -u "conda activate double_env_6;python source/approx_run_mnist.py --model resnet18 --task mnist --epochs 1 --batch-size 64 --lr 0.001 --seed {2} --activation {1} --a $a --b $b --c $c --d $d" ::: "${activations[@]}" ::: $seed
done
