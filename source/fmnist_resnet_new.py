...
class HyperbolicCosineRoot(nn.Module):
    def forward(self, x):
        return torch.sign(x) * torch.sqrt(torch.cosh(x) - 1)

class LogarithmicSigmoid(nn.Module):
    def forward(self, x):
        return torch.log1p(torch.exp(x)) / (1 + torch.log1p(torch.exp(x)))
...
class FashionCNN(nn.Module):
    def __init__(self, activation_func='ReLU'):
        ...
        self.activation_functions = {
            ...
            'SEL': SmoothExponentialLinear(),
            'HCR': HyperbolicCosineRoot(),
            'LogSig': LogarithmicSigmoid(),
        }
    ...
    def forward(self, x):
        ...
        out = self.fc1(out)
        out = self.drop(out)
        out = self.activation(out)  # Apply activation after fc1
        out = self.fc2(out)
        out = self.activation(out)  # Apply activation after fc2
        out = self.fc3(out)
...
# Save data
save_file_name = f"./saves/new_fashionmnist.json"
