...
def run_comparison(input_size, num_runs=1000):
    ...
    delu = DELU().to(device)
    transformers_gelu = GELUActivation().to(device)
    ...
    for _ in range(num_runs):
        ...
        # Transformers GELU forward and backward
        _, gelu_forward_time = measure_time(lambda: transformers_gelu(input_tensor))
        gelu_output = transformers_gelu(input_tensor)
    ...
    return [
        ...
        ["Transformers GELU", avg_gelu_forward, avg_gelu_backward, 0, 0]
    ]
